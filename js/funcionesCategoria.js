...
function postCategoria(){
    if($("#name").val().length==0 || $("#description").val().length==0){
        ...
        $.ajax({
            url:"http://132.145.223.170:8080/api/Category/save",
            type:"POST",
            datatype:"JSON",
...
function putCategoria(idBotonActualizar){
    ...
    $.ajax({
        url:"http://132.145.223.170:8080/api/Category/update",
        type:"PUT",
        datatype:"JSON",
...
function deleteCategoria(idBotonBorrar){
    const swalWithBootstrapButtons = Swal.mixin({
      ...
      swalWithBootstrapButtons.fire({
        title: 'Estás seguro de borrar la categoria?',
        ...
        $.ajax({
            url:"http://132.145.223.170:8080/api/Category/"+idBotonBorrar,
            type:"DELETE",
            datatype:"JSON",
            contentType:"application/JSON",
...
function pintarCategoria(respuesta){
    ...
    for(i=0;i<respuesta.length;i++){
        myTable+="<tr class='bg-white border-b'>";
